...
Backbone.couchConnector = {
	...
	getCollectionFromModel : function(model){
        ...
        get_options_from_model: function(model){
	    if(!(model && model.url_opts))
		return '';
	    if (model.url_opts.length > 0){
		var o = model.url_opts;
		var fst = o.substring(0,1);
		if (fst == '?')
		    return o;
		else
		    return '?'+o;
	...
	readCollection : function(coll, _success, _error){
		...
		var collection = this.getCollectionFromModel(coll);
	        var opts = this.get_options_from_model(coll);
